...
void VBOTerrain::init(VBORenderManager* rendManager, const glm::vec2& size) {
	...
	if (!initialized) {
		...
		for (int i = 0; i < numLevels; ++i) {
			fileNames[i] = "data/textures/0" + QString::number(i+1) + "_terrain.png";
		}
	...
	{
		std::cout << "rendManager->minPos x is " << rendManager->minPos.x() << std::endl;
		std::cout << "rendManager->minPos y is " << rendManager->minPos.y() << std::endl;
	}
	// VERTEX
	...
	for (int r = 0; r < size.y; r ++) {
		for (int c = 0; c < size.x; c ++) {
			QVector3D pos = QVector3D(c * step, r * step, 0) + 4 * rendManager->minPos;
			//vert[count] = Vertex(pos, pos/100.0f);
			//new texture
			vert[count] = Vertex(pos, QVector3D(c / size.x, r / size.y, 0));
			count++;
		}
	}
	glGenBuffers(1, &vbo);
...
void VBOTerrain::render() {
	...
	glUniform4f(glGetUniformLocation(rendManager->program, "terrain_size"), 
		4 * rendManager->minPos.x(),
		4 * rendManager->minPos.y(),
		4 * (rendManager->maxPos.x() - rendManager->minPos.x()),
		4 * (rendManager->maxPos.y() - rendManager->minPos.y())
		);//tex0: 0
