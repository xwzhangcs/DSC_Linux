...
static std::vector<QString> hroofTex;
static std::vector<QVector3D> hRoofScale;
...
void addTexConvexPolyHouse(VBORenderManager& rendManager, const QString& geoName, const QString& textureName, int shaderMode, const Loop3D& pos, const QColor& col, float zShift, const QVector3D& texScale, bool flipped = false) {
	if (pos.size() < 3) return;
	...
	std::vector<Vertex> vert(pos.size() * 3);
	// get the center
	...
	for (int i = 0; i < pos.size(); ++i) {
		...
		std::vector<QVector3D> texP;
		//first
		points.push_back(QVector3D(pos[i].x(), pos[i].y(), pos[i].z() + zShift));
		if (texScale.x() == 0 && texScale.y() == 0) {
			QVector3D cP2 = xformMat * QVector3D(pos[i].x(), pos[i].y(), 0);
			texP.push_back(QVector3D((cP2.x() - minX) / size.x(), (cP2.y() - minY) / size.y(), 0.0f));
		}
		else {
			QVector3D cP2 = xformMat * QVector3D(pos[i].x(), pos[i].y(), 0);
		...
		//second
		points.push_back(QVector3D(pos[next].x(), pos[next].y(), pos[next].z() + zShift));
		if (texScale.x() == 0 && texScale.y() == 0) {
			QVector3D cP2 = xformMat * QVector3D(pos[next].x(), pos[next].y(), 0);
			texP.push_back(QVector3D((cP2.x() - minX) / size.x(), (cP2.y() - minY) / size.y(), 0.0f));
		}
		else {
			QVector3D cP2 = xformMat * QVector3D(pos[next].x(), pos[next].y(), 0);
		...
		//third
		points.push_back(QVector3D(center.x(), center.y(), center.z() + zShift + 10.0f));
		if (texScale.x() == 0 && texScale.y() == 0) {
			QVector3D cP2 = xformMat * QVector3D(center.x(), center.y(), 0);
			texP.push_back(QVector3D((cP2.x() - minX) / size.x(), (cP2.y() - minY) / size.y(), 0.0f));
		}
		else {
			QVector3D cP2 = xformMat * QVector3D(center.x(), center.y(), 0);
		...
		vert[i * 3 + 0] = Vertex(points[0], col, norm, texP[0]);
		vert[i * 3 + 1] = Vertex(points[1], col, norm, texP[1]);
		vert[i * 3 + 2] = Vertex(points[2], col, norm, texP[2]);
...
void calculateColumnContour(const std::vector<QVector3D>& currentContour, std::vector<QVector3D>& columnContour, std::vector<int>& windows) {
	columnContour.clear();
	windows.clear();
	//std::cout << "currentContour.size() is " << currentContour.size() << std::endl;
	for (int i = 0; i < currentContour.size(); ++i) {
		...
		if (len > 7.0f) {
			...
			windows.push_back(0);
			//std::cout << "pos1 is (" << pos1.x() << ", " << pos1.y() << ", " << pos1.z() << ")" << std::endl;
			pos1 = pos1 + 0.5f * dirV;
			/*std::cout << "dirV is (" << dirV.x() << ", " << dirV.y() << ", " << dirV.z() << ")" << std::endl;
			std::cout << "perDir is (" << perDir.x() << ", " << perDir.y() << ", " << perDir.z() << ")" << std::endl;
			std::cout << "numWindows is " << numWindows << std::endl;*/
			for (int j = 0; j < numWindows; ++j) {
				/*{
					...
					windows.push_back(0);
					columnContour.push_back(pos1 + windowWidth * 2.0f * dirV);
					windows.push_back(0);
...
void addFirstFloor(VBORenderManager& rendManager, const std::vector<QVector3D>& columnContour, const std::vector<int>& windows, int randomFacade, const QVector3D& randN, float uS, float vS, float height, float floorHeight, const QVector3D& windowRandomSize) {
	...
	//std::cout << "first floor is "<<columnContour.size() << std::endl;
...
void GeoBuilding::initBuildingsTex() {
	...
	// 4. house roofs
	QDir directoryHR(pathName + "hroof/");
	list = directoryHR.entryList(nameFilter, QDir::Files);
	for (int lE = 0; lE < list.size(); ++lE) {
		hroofTex.push_back(pathName + "hroof/" + list[lE]);
		hRoofScale.push_back(QVector3D(1.0f, 1.0f, 0));
	}
...
void GeoBuilding::generateBuilding(VBORenderManager& rendManager, Building& building) {
	...
	if (building.bldType == 1) {
		...
		float base_elv = min_elev;
		//if (building.baseLevel > 0)
		base_elv = min_elev + building.baseLevel;
		//float baseHeight = 200 + 2.8f;
		...
		addExtrudedGeom(rendManager, "3d_building", building.buildingFootprint.contour, bldgColor, base_elv, baseHeight);
		addConvexPoly(rendManager, "3d_building", building.buildingFootprint.contour, bldgColor, base_elv + baseHeight);
		/// Add columns
		...
		// First floor
		addFirstFloor(rendManager, columnContour, windows, randomFacade, randN, uS, vS, base_elv + baseHeight, firstFloorHeight, windowRandSize);

		// Upper floors
		addColumnGeometry(rendManager, columnContour, windows, randomFacade, randN, uS, vS, base_elv + baseHeight + firstFloorHeight, building.numStories - 1, storyHeight, windowRandSize);

		////////////////////////////
		// ROOF	
		addRoof(rendManager, roofOffCont, bldgColor, base_elv + buildingHeight, 1.0f);
	}
	// L building
	if (building.bldType == 2){
		float baseHeight = max_elev - min_elev + 2.8f;
		float firstFloorHeight = 4.0f;
		int floors_base = 0.8 * building.numStories;
		int floors_upper = building.numStories - floors_base;
		float baseBuildingHeight = baseHeight + firstFloorHeight + (floors_base - 1) * storyHeight;
		float buildingHeight = baseHeight + firstFloorHeight + (building.numStories - 1) * storyHeight;

		Loop3D polygon = building.buildingFootprint.contour;
		Loop3D polygon_left_half;
		Loop3D polygon_right_half;
		if (polygon.size() == 4){

			polygon_left_half.push_back(polygon[0]);
			polygon_left_half.push_back(0.5 * (polygon[0] + polygon[1]));
			polygon_left_half.push_back(0.5 * (polygon[2] + polygon[3]));
			polygon_left_half.push_back(polygon[3]);

			polygon_right_half.push_back(0.5 * (polygon[0] + polygon[1]));
			polygon_right_half.push_back(polygon[1]);
			polygon_right_half.push_back(polygon[2]);
			polygon_right_half.push_back(0.5 * (polygon[2] + polygon[3]));
		...
		// Upper floors
		addColumnGeometry(rendManager, columnContour, windows, randomFacade, randN, uS, vS, min_elev + baseHeight + firstFloorHeight, floors_base - 1, storyHeight, windowRandSize);

		////////////////////////////
		// ROOF	
		int randomRoof = qrand() % roofTex.size();
		//addRoof(rendManager, roofOffCont, bldgColor, min_elev + buildingHeight, 1.0f);
		addTexConvexPoly(rendManager, "3d_building", roofTex[randomRoof], 2 | mode_Lighting, polygon, bldgColor, min_elev + baseBuildingHeight, QVector3D(0, 0, 0));
		/// Add columns
		if (building.buildingFootprint.contour.size() == 4){
			columnContour.clear();
			windows.clear();
			calculateColumnContour(polygon_left_half, columnContour, windows);
			// Upper floors
			addColumnGeometry(rendManager, columnContour, windows, randomFacade, randN, uS, vS, min_elev + baseBuildingHeight, floors_upper, storyHeight, windowRandSize);
			// ROOF	
			addTexConvexPoly(rendManager, "3d_building", roofTex[randomRoof], 2 | mode_Lighting, polygon_left_half, bldgColor, min_elev + buildingHeight, QVector3D(0, 0, 0));
		}

	}
	// another L
	if (building.bldType == 3){
		float baseHeight = max_elev - min_elev + 2.8f;
		float firstFloorHeight = 4.0f;
		int floors_base = 0.8 * building.numStories;
		int floors_upper = building.numStories - floors_base;
		...
		if (polygon.size() == 4){
			...
			polygon_tmp.push_back(polygon[0]);
			polygon_tmp.push_back(polygon[1]);
			polygon_tmp.push_back(polygon[2]);
			polygon_tmp.push_back(0.5 * (polygon[2] + polygon[3]));
			polygon_tmp.push_back(0.5 * (0.5 * (polygon[0] + polygon[3]) + 0.5 * (polygon[1] + polygon[2])));
			polygon_tmp.push_back(0.5 * (polygon[3] + polygon[0]));
			polygon = polygon_tmp;
		}
		...
		///////////////////////////
		// Add Base
		addExtrudedGeom(rendManager, "3d_building", polygon, bldgColor, min_elev, baseHeight);
		addConvexPoly(rendManager, "3d_building", polygon, bldgColor, min_elev + baseHeight);

		/// Add columns
		std::vector<QVector3D> columnContour;
		std::vector<int> windows;
		calculateColumnContour(polygon, columnContour, windows);
	...
	// house
	if (building.bldType == 4){
		Loop3D polygon = building.buildingFootprint.contour;

		// add box
		addTexExtrudedGeom(rendManager, "3d_building", polygon, building.color, min_elev, building.numStories * storyHeight);
		
		// add roof
		int randomRoof = qrand() % hroofTex.size();
		if (polygon.size() == 4){
			addTexConvexPolyHouse(rendManager, "3d_building", hroofTex[randomRoof], 2 | mode_Lighting, polygon, building.color, min_elev + building.numStories * storyHeight, QVector3D(0, 0, 0));
		}
		else{
			addTexConvexPoly(rendManager, "3d_building", hroofTex[randomRoof], 2 | mode_Lighting, polygon, building.color, min_elev + building.numStories * storyHeight, QVector3D(0, 0, 0));
		}
	}
}
