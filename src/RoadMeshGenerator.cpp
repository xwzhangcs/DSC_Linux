...
void RoadMeshGenerator::generateRoadMesh(VBORenderManager& rendManager, RoadGraph& roads) {
	...
	std::cout << "major_road_width is " << G::getFloat("major_road_width") << std::endl;
	//////////////////////////////////////////
	// POLYLINES
	{
		...
		for (boost::tie(ei, eiEnd) = boost::edges(roads.graph); ei != eiEnd; ++ei) {
			...
			if (roads.graph[*ei]->type == RoadEdge::TYPE_AVENUE) {
				hWidth = roads.graph[*ei]->getHalfWidth(G::getFloat("major_road_width") * 0.5f);
			}
			else if (roads.graph[*ei]->type == RoadEdge::TYPE_SECONDARY){
				hWidth = roads.graph[*ei]->getHalfWidth(G::getFloat("second_road_width") * 0.5f);
			}
			else {
				hWidth = roads.graph[*ei]->getHalfWidth(G::getFloat("minor_road_width") * 0.5f);
			}
			...
			if (outDegree == 0) {
				...
				for (boost::tie(oei, oeend) = boost::out_edges(*vi, roads.graph); oei != oeend; ++oei) {
					...
					if (roads.graph[*oei]->type == RoadEdge::TYPE_AVENUE) {
						rad = roads.graph[*oei]->getHalfWidth(G::getFloat("major_road_width") * 0.5f);
					}
					else if (roads.graph[*oei]->type == RoadEdge::TYPE_SECONDARY){
						rad = roads.graph[*oei]->getHalfWidth(G::getFloat("second_road_width") * 0.5f);
					}
					else {
						rad = roads.graph[*oei]->getHalfWidth(G::getFloat("minor_road_width") * 0.5f);
					}
				...
				for (int eN = 0; eN < edgeAngleOut.size(); eN++) {
					...
					else if (roads.graph[edgeAngleOut[eN].second]->type == RoadEdge::TYPE_SECONDARY) {
						ed1W = roads.graph[edgeAngleOut[eN].second]->getWidth(G::getFloat("second_road_width") * 0.5f);
					}
					else {
					...
					else if (roads.graph[edgeAngleOut[lastEdge].second]->type == RoadEdge::TYPE_SECONDARY) {
						ed2WL = roads.graph[edgeAngleOut[lastEdge].second]->getWidth(G::getFloat("second_road_width") * 0.5f);
					}
					else {
					...
					else if (roads.graph[edgeAngleOut[nextEdge].second]->type == RoadEdge::TYPE_SECONDARY) {
						ed2WR = roads.graph[edgeAngleOut[nextEdge].second]->getWidth(G::getFloat("second_road_width") * 0.5f);
					}
					else {
...
void RoadMeshGenerator::generate2DRoadMesh(VBORenderManager& renderManager, RoadGraph& roads) {
	...
	// EDGES
	{
		...
		for (boost::tie(ei, eend) = boost::edges(roads.graph); ei != eend; ++ei) {
			...
			float halfWidth;
			if (roads.graph[*ei]->type == RoadEdge::TYPE_AVENUE) {
				halfWidth = roads.graph[*ei]->getHalfWidth(G::getFloat("major_road_width") * 0.5f);
			}
			if (roads.graph[*ei]->type == RoadEdge::TYPE_SECONDARY) {
				halfWidth = roads.graph[*ei]->getHalfWidth(G::getFloat("second_road_width") * 0.5f);
			}
			else {
				halfWidth = roads.graph[*ei]->getHalfWidth(G::getFloat("minor_road_width") * 0.5f);
			}
	...
	// INTERSECTIONS
	{
		...
		for (boost::tie(vi, vend) = boost::vertices(roads.graph); vi != vend; ++vi) {
			...
			for (boost::tie(oei, oeend) = boost::out_edges(*vi, roads.graph); oei != oeend; ++oei) {
				...
				if (roads.graph[*oei]->type == RoadEdge::TYPE_AVENUE) {
					halfWidth = roads.graph[*oei]->getHalfWidth(G::getFloat("major_road_width") * 0.5f);
				}
				else if (roads.graph[*oei]->type == RoadEdge::TYPE_SECONDARY) {
					halfWidth = roads.graph[*oei]->getHalfWidth(G::getFloat("second_road_width") * 0.5f);
				}
				else {
					halfWidth = roads.graph[*oei]->getHalfWidth(G::getFloat("minor_road_width") * 0.5f);
				}
