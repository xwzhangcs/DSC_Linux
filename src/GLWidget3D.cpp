...
void GLWidget3D::wheelEvent(QWheelEvent* e) {
	camera.changeXYZTranslation(0, 0, -e->delta() * 0.5);
...
void GLWidget3D::paintGL() {
	...
	//std::cout << "camera.fovy is " << camera.fovy << std::endl;
	updateCamera();
...
void GLWidget3D::drawScene(int drawMode) {
	if (G::getBool("shader2D")) {
		...
		if (drawMode == 0) {	// Regular rendering
			...
			glUniform1i(glGetUniformLocation(vboRenderManager.program,"shadowState"), 0);// SHADOW: From light
		...
		// record the camera views
		std::ofstream out;
		out.open("camera/view_sequence.txt", std::ios_base::app);
		out << camera.rot.x() << "," << camera.rot.y() << "," << camera.rot.z() << ",";
		out << camera.pos.x() << "," << camera.pos.y() << "," << camera.pos.z() << ",";
		out << camera.lookAt.x() << "," << camera.lookAt.y() << "," << camera.lookAt.z() << std::endl;
		out.close();
	}
}
