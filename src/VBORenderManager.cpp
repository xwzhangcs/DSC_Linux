...
bool VBORenderManager::addStaticGeometry2WithHole(const QString& geoName, const std::vector<QVector3D>& pos, const std::vector<std::vector<QVector3D>>& holes, float zShift, const QString& textureName, int shaderMode, const QVector3D& texScale, const QColor& color){
	if (pos.size() < 3) return false;
	if (this == NULL || geoName == NULL)
		return false;
	if (!geoName.contains("block") && !geoName.contains("parcel") && !geoName.contains("road") && !geoName.contains("park")){
		std::cout << "geoName is " << geoName.toUtf8().constData() << std::endl;
	}
	QString geoTmp = geoName;
	int geoSize = geoName.size();
	...
	for (int i = 0; i < pos.size(); ++i) {
		...
		float distance = 0.0f;
		distance = (pos[i].x() - pos[(i + 1) % pos.size()].x())*(pos[i].x() - pos[(i + 1) % pos.size()].x()) + (pos[i].y() - pos[(i + 1) % pos.size()].y()) * (pos[i].y() - pos[(i + 1) % pos.size()].y());
		distance = sqrt(distance);
		if (distance < 1){
			//std::cout << "come here" << std::endl;
			return false;
		}
		//if (pos[i].x() > 781.461 && pos[i].x() < 781.463 && pos[i].y() > 0.0520284 && pos[i].y() < 0.0520286)
		//	return false;
		//if (pos[i].x() > -2552 && pos[i].x() < -2551.96 && pos[i].y() > 771.37 && pos[i].y() < 771.373)
		//	return false;
		//if (pos[i].x() > 3416.3 && pos[i].x() < 3416.32 && pos[i].y() > 1095.15 && pos[i].y() < 1095.17)
		//	return false;
		//if (pos[i].x() > -2552.25 && pos[i].x() < -2552.2 && pos[i].y() > 775.523 && pos[i].y() < 775.525)
		//	return false;
		//if (pos[i].x() > -2551.98 && pos[i].x() < -2551.94 && pos[i].y() > 770.66 && pos[i].y() < 770.69)
		//	return false;
		if (pos[i].x() > -0.076598 && pos[i].x() < -0.07659 && pos[i].y() > -6.65425 && pos[i].y() < -6.6542)
			return false;
		if (pos[i].x() > 524.148 && pos[i].x() < 524.15 && pos[i].y() > 184.976 && pos[i].y() <184.979)
			return false;
		if (pos[i].x() > 1605.96 && pos[i].x() < 1605.99 && pos[i].y() > -1090.82 && pos[i].y() <-1090.80)
			return false;
		if (i == 0 /*&& !G::getBool("shader2D")*/){
			//std::cout << "pos " << i << " (" << pos[i].x() << ", " << pos[i].y() << ")" << std::endl;
		}
			//std::cout << "pos " << i << " (" << pos[i].x() << ", " << pos[i].y() <<")"<<std::endl;
	}
